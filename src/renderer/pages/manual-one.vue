  ...
  <div>
    ...
    <div class="menu">
      ...
      <div class="btn-menu">
        ...
        <div class="spacer">
          Хотя содержимое видео кажется одинаковым, уверяю, что источники разные. Просто некогда было брать 16 разных видео (особенно локально)
        </div>
        <div
          class="btn"
          @click='back'
        >
          <ButtonCommon title="Назад"/>
        </div>
...
export default {
  ...
  mounted() {
    let t = this;
    t.calc_size();
    window.addEventListener('onresize2', function(event) {
      t.calc_size();
    });
  },
